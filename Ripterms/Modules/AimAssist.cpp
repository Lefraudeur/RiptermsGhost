...
void Ripterms::Modules::AimAssist::onEvent(Ripterms::Event* event)
{
	if (event->type == Ripterms::Event::Type::PRE_MOTION)
	{
		...
		for (EntityPlayer& target : Ripterms::cache->playerEntities.toVector<EntityPlayer>())
		{
			if (target.isEqualTo(cache->thePlayer))
				continue;
			Maths::Vector3d target_position = target.getRenderPosition();
		...
		if (selected_target.isValid())
		{
			...
			std::uniform_int_distribution<> randbb(0, 40);
			float maxDelta = std::abs(Ripterms::Maths::cropAngle180((maxYaw - minYaw))) / 2.0f - float(randbb(gen)) / 10.0f;
			...
			float maxPitchDelta = std::abs(maxPitch - minPitch) / 2.0f - float(randbb(gen)) / 10.0f;
